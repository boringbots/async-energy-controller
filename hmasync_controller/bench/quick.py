...
async def resolve_quick_model(
    ...
    *,
    ...
    (`bench prism` matches both against the GGUF the server names in `GET
    /v1/models` before measuring). They exist because attach mode has nothing
    to read them from: the branch below records the server's own id and a null
    quantization precisely so that it never guesses. Ollama needs no
    equivalent -- its pinned tag and manifest digest already name the weights.
