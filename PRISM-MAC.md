...
> **Superseded, kept for the framing.** The wave that ran on 2026-09-23/24
> used the attach-mode `bench prism` in `hmasync_controller/bench/prism.py`,
> which identifies whichever pinned rung llama-server is serving. It covered
> eight rungs, not the five named below, and wrote no
> `prism-mac-summary/1` files -- so `scripts/run-prism-wave-mac.sh` and
> `scripts/compare-prism-rungs.py`, and the `--rung`/`--list` flags the
> commands below use, describe a code path that is no longer in the tree.
> The reversal question this document frames is still the reason the wave
> exists, and its 27B PTQ1_0-vs-PQ2_0 pair did run. Read it for that; run the
> wave from the module docstring in `bench/prism.py`.
